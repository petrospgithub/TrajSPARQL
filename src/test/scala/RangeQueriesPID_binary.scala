...
class RangeQueriesPID_binary extends FunSuite {
...
/*
  def rangeArrStruct_index_pid(): Long = {
    ...
    val sql = " SELECT * FROM " +
      "(SELECT ST_IndexIntersects(MbbConstructor( %s, %s, %s, %s, CAST(%s as BIGINT), CAST(%s as BIGINT) ),tree, 0.1, 0.1, 0.1, 0.1, 0, 0) FROM partition_index_imis400) as b INNER JOIN trajectories_imis400_pid as a ON (b.trajectory_id=a.pid) ".format(env.get.getMinX, env.get.getMaxX, env.get.getMinY, env.get.getMaxY, env.get.getMinT, env.get.getMaxT) +
...
*/
  def rangeBinary_index_pid(): Long = {
    ...
    val sql = " SELECT * FROM " +
      "(SELECT ST_IndexIntersectsBinary(MbbConstructorBinary( %s, %s, %s, %s, CAST(%s as BIGINT), CAST(%s as BIGINT) ),tree, 0.1, 0.1, 0.1, 0.1, 0, 0) FROM partition_index_imis400_binary) as b INNER JOIN trajectories_imis400_binary_pid as a ON (b.trajectory_id=a.pid) ".format(env.get.getMinX, env.get.getMaxX, env.get.getMinY, env.get.getMaxY, env.get.getMinT, env.get.getMaxT) +
