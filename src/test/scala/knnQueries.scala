...
class knnQueries extends FunSuite {
  ...
  def knnArrStruct_BF(): Long = {
    ...
    val rs = stmt.executeQuery("select rowId from oasa400_temp distribute by rand() sort by rand() limit 1")
    ...
    val sql = " SELECT a.rowId, ToOrderedList_arr( DTW_arr(a.trajectory, b.trajectory, 50, 'Euclidean', 21600, 21600), b.rowId, '-k -3', a.trajectory, b.trajectory) " +
      "FROM (SELECT * FROM oasa400_temp WHERE rowId=" + id.get + ") as a CROSS JOIN oasa400_temp as b " +
      " GROUP BY a.rowId  "
  ...
  def knnArrStruct_index(): Long = {
    ...
    val sql = " SELECT " +
      " trajectories_oasa400.rowId, ToOrderedList_arr( DTW_arr(trajectories_oasa400.trajectory, final.trajectory, 50, 'Euclidean', 21600, 21600), final.trajectory_id, '-k -3', trajectories_oasa400.trajectory, final.trajectory) " +
      " FROM ( SELECT IndexTrajKNN_arr(c.trajectory, d.tree, 2000.0, 21600, 21600, c.rowId) " +
      " FROM " +
      " ( SELECT IndexTrajKNN_arr(a.trajectory,b.tree, 2000.0, 21600, 21600, a.rowId) FROM ( SELECT * FROM trajectories_oasa400 where rowId=" + id.get + " ) as a JOIN partition_index_oasa400 as b ) as c " +
      " INNER JOIN index_oasa400 as d ON (c.trajectory_id=d.id) ) as final INNER JOIN trajectories_oasa400 ON (final.trajectory_id=trajectories_oasa400.rowId) " +
      " GROUP BY trajectories_oasa400.rowId "
  ...
  def knnBinary_BF(): Long = {
    ...
    val sql = "SELECT a.rowId, ToOrderedListBinary( DTW_binary(a.trajectory, b.trajectory, 50, 'Euclidean', 21600, 21600), b.rowId, '-k -3', a.trajectory, b.trajectory) " +
      " FROM (SELECT * FROM trajectories_oasa400_binary WHERE rowId=" + id.get + ") as a CROSS JOIN trajectories_oasa400_binary as b " +
      " GROUP BY a.rowId "
  ...
  def knnBinary_index(): Long = {
    ...
    val sql = " SELECT " +
      " trajectories_oasa400_binary.rowId, ToOrderedListBinary( DTW_binary(trajectories_oasa400_binary.trajectory, final.trajectory, 50, 'Euclidean', 21600, 21600), final.trajectory_id, '-k -3', trajectories_oasa400_binary.trajectory, final.trajectory) " +
      " FROM ( SELECT IndexTrajKNN_binary(c.trajectory, d.tree, 2000.0, 21600, 21600, c.rowId) " +
      " FROM " +
      " ( SELECT IndexTrajKNN_binary(a.trajectory,b.tree, 2000.0, 21600, 21600, a.rowId) FROM ( SELECT * FROM trajectories_oasa400_binary where rowId=" + id.get + " ) as a JOIN partition_index_oasa400_binary as b ) as c " +
      " INNER JOIN index_oasa400_binary as d ON (c.trajectory_id=d.id) ) as final INNER JOIN trajectories_oasa400_binary ON (final.trajectory_id=trajectories_oasa400_binary.rowId) " +
      " GROUP BY trajectories_oasa400_binary.rowId "
  ...
  def knnBinaryTraj(): Long = {
    ...
    val sql = "SELECT IndexStoreTrajKNN_binary(c.trajectory, d.tree, 2000.0, 21600, 21600, c.rowId, 'DTW', 'Euclidean', 3, 50, 0.0, 0 ) " +
      " FROM ( SELECT IndexTrajKNN_binary(a.trajectory,b.tree, 2000.0, 21600, 21600, a.rowId) FROM (SELECT * FROM trajectories_oasa400_binary where rowId=" + id.get + ") as a JOIN partition_index_oasa400_binary as b ) as c" +
      " INNER JOIN index_oasa400_binaryTraj as d ON (c.trajectory_id=d.id) "
  ...
  test("Thesis range queries") {
    ...
    */
    ...
    println("Binary mean time: " + buffer_knnBinaryTraj.sum / buffer_knnBinaryTraj.length.toDouble)
